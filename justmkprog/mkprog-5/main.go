...
1. Main package with main.go file
...
Ensure that the generated project structure is well-organized, follows Go best practices, and implements the functionality described in the project description.`, systemPrompt, description)
...
func createProjectFiles(projectStructure string) error {
	...
	for _, file := range files {
		...
		parts := strings.SplitN(strings.TrimSpace(file), "\n", 2)
		if len(parts) != 2 {
			continue
		}
